...
const mysql2 = require('../index')({
  ...
  config: {
    host     : 'localhost',
    database : 'proleaguer_dev',
    user     : 'root',
    password : 'root',
    port     : '8889'
  },
  onConnect: (client) => { console.log('MYSQL2 Connected:', client.threadId) },
  onClose: () => { console.log('MYSQL2 Connection Closed') },
  onError: (e) => { console.log('MYSQL2 Connection Error:',e.message) },
  onRetry: (e,retries,sleep) => { console.log('RETRY2',retries,sleep) },
  onKill: (zombie) => { console.log('Kill2:',zombie) },
  onKillError: (e) => { console.log('Kill Error2:',e.message) }
})
...
const handler = async () => {
  ...
  try {
    ...
    let results2 = await mysql2.query('SELECT * FROM golfers LIMIT 0,1')
